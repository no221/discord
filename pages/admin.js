...
export default function Admin(){
  ...
  const monthlyBuy = data.purchases.filter(p=> new Date(p.createdAt).getMonth() === new Date().getMonth()).length

  // most bought
  const countByProduct = {}
  data.purchases.forEach(p=>{ countByProduct[p.productId] = (countByProduct[p.productId]||0) + p.qty })
